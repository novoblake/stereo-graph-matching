    ...
    def compute_cost_matrix(self, node_feats_left: torch.Tensor, right_feats_candidates: torch.Tensor, left_img: np.ndarray, segments: np.ndarray, masks: List[np.ndarray], candidates: List[int]) -> torch.Tensor:
        ...
        for i, mask in enumerate(masks):
            ...
            if rx < 2 or rx > w-3:
                ...
                patchR = (right_img[cy-2:cy+3, rx-2:rx+3] * 255).astype(np.uint8)
            for k, d in enumerate(candidates):
                if cx - d < 2 or cx + 3 > w - 1 or cy - 2 < 0 or cy + 3 > h - 1:
                    patch_costs[i, k] = 1e6
                else:
                    patchL = (left_img[cy - 2:cy + 3, cx - 2:cx + 3] * 255).astype(np.uint8)
                    patchR = (np.roll(right_img[cy-2:cy+3, cx-d-2:cx-d+3]*255).astype(np.uint8)
...
def plane_prior_loss(disp, segments):
    disp_np = disp.detach().cpu().numpy()
    H, W = disp_np.shape
    loss = 0.0
    count = 0
    for sid in range(segments.max()+1):
        mask = (segments == sid)
        ys, xs = np.where(mask)
        ...
        d_fit = (X @ np.array([a,b,c]))
        loss += np.mean(np.abs(Y - d_fit))
        count += 1
    return torch.tensor(loss/count, device=disp.device)



def lr_consistency_loss(dL, dR):
    # warp right disparity into left view
    B,_,H,W = dL.shape
    # compute warped coords: xR = xL - dL
    xx, yy = torch.meshgrid(
        torch.arange(W, device=dL.device),
        torch.arange(H, device=dL.device),
        indexing='xy'
    )
    xxR = (xx[None] - dL).clamp(0, W-1)
    grid = torch.stack([(xxR/W)*2-1, (yy/W)*2-1], dim=-1)
    dR_warp = F.grid_sample(dR, grid, padding_mode='border')
    return (dL - dR_warp).abs().mean()
...
def train_loop(train_root, val_root, device='cuda'):
    ...
    for epoch in range(1, 31):  # 30 epochs
        ...
        for (lpath, rpath, dpath) in loader:
            ...
            left_tensor = torch.tensor(left.transpose(2,0,1)[None], device=device)
            right_tensor = torch.tensor(right.transpose(2,0,1)[None], device=device)
            disp_tensor = out['pixel_disp_refined'][None,None] / self.dmax

            loss_photo = photometric_loss(left_tensor, right_tensor, disp_tensor)
            loss_lr = lr_consistency_loss(
                        out['pixel_disp_refined'][None,None],
                        right_disp_pred
                    )
            loss_plane = plane_prior_loss(out['pixel_disp_refined'], segments)
            ...
            loss_lr = lr_consistency_loss(left_disp, right_disp)

            loss = loss_nodes + 0.1 * loss_lr

            loss_plane = 0.05 * plane_prior_loss(out_disp_t, segments)
            loss = loss_nodes + loss_plane


            pred_valid = pred_nodes[mask_valid]           # this keeps requires_grad=True
            loss_l1 = F.l1_loss(pred_valid, gt_nodes)
            loss = loss_l1 + 0.1*loss_photo + 0.1*loss_lr + 0.05*loss_plane
